...
class EnterOtherStoragePremisesControllerSpec
  ...
  val controller =
    new EnterOtherStoragePremisesController(commonDependencies, views, mockActions, mockAppConfig)(mockMcc)

  "load" should {
    "Render the OtherStoragePremises page" when {
        ...
        "The form is filled out correctly" in {
          ...
          val request = FakeRequest()
            .withFormUrlEncodedBody(
              "storagePremise_address.Line1"    -> "44 Test town",
              "storagePremise_address.postcode" -> "AB1 2YZ",
              "isThirdParty"                    -> "true",
              "addMore"                         -> "false"
            )
            .withMethod("POST")

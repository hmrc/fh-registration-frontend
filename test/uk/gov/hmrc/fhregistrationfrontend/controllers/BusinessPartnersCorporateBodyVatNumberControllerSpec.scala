...
class BusinessPartnersCorporateBodyVatNumberControllerSpec extends ControllerSpecWithGuiceApp with ActionsMock {
  ...
  val controller =
    new BusinessPartnersCorporateBodyVatNumberController(commonDependencies, views, mockActions, mockAppConfig)(mockMcc)
  ...
  "load" should {
    "Render the businessPartnersCorporateBodyVatNumber page" when {
      "the new business partner pages are enabled" in {
        ...
        page.title() should include("Does the corporate body have a UK VAT registration number?")
        reset(mockActions)
      }
    }

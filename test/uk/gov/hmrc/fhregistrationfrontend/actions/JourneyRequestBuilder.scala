...
trait JourneyRequestBuilder extends ActionSpecBase {
  ...
  def examplePageData(page: Page[_]): Page[_ >: ContactPerson with MainBusinessAddress with CompanyRegistrationNumber with DateOfIncorporation with NationalInsuranceNumber with TradingName with VatNumber with ListWithTrackedChanges[CompanyOfficer] with ListWithTrackedChanges[BusinessPartner] with BusinessStatus with ImportingActivities with BusinessCustomers with OtherStoragePremises <: Product] =
    page.id match {
      case contactPersonPage.id => page.asInstanceOf[Page[ContactPerson]] withData FormTestData.contactPerson
      case mainBusinessAddressPage.id =>
        page.asInstanceOf[Page[MainBusinessAddress]] withData FormTestData.mainBusinessAddress

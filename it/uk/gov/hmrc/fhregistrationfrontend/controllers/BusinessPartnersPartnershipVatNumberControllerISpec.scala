...
class BusinessPartnersPartnershipVatNumberControllerISpec extends Specifications with TestConfiguration {

  def route(mode: Mode): String = routes.BusinessPartnersPartnershipVatNumberController.load(1, mode).url.drop(6)

  def userAnswersWithPageData(formAnswers: VatNumber) =
    emptyUserAnswers
  ...
  List(NormalMode, CheckMode).foreach { mode =>
    ...
    List(NormalMode, CheckMode).foreach { mode =>
      val partnershipUtrPage: String = routes.BusinessPartnersPartnershipUtrController.load(1, mode).url
            ...
            userAnswers.zipWithIndex.foreach { answers =>
              s"redirect to the correct page and $userAnswersAction userAnswers ${answers._2}" when {
                "Yes is selected and Vat Number supplied, and legal entity type is Partnership" in {
                  ...
                  whenReady(result) { res =>
                    res.status mustBe 303
                    res.header(HeaderNames.LOCATION) mustBe Some(partnershipUtrPage)
                  }
                }

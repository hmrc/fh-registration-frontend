     ...
     "return 200" when {
       "the user selects no" in {
         ...
         WsTestClient.withClient { client =>
           ...
           whenReady(result) { res =>
             res.status mustBe 200
             res.body must include("Form submitted, with result:MainBusinessAddress(Less than 3 years,Some(false),None,None)")
           }
         }
       }
       ...
       "the user selects yes and correctly fills out the form" in {
         ...
         WsTestClient.withClient { client =>
           ...
           whenReady(result) { res =>
             res.status mustBe 200
             res.body must include("Form submitted, with result:MainBusinessAddress(Less than 3 years,Some(true),Some(Address(line 1,None,None,None,AA1 1YZ,None,None)),Some(2022-10-01))")
           }
         }
       }
     }
      ...
      "return 400" in {
        ...
        WsTestClient.withClient { client =>
          ...
          whenReady(result) { res =>
            ...
            val page = Jsoup.parse(res.body)
            page.title must include(pageTitle)
            page.getElementsByClass("govuk-list govuk-error-summary__list").text() must include("Select how long the business has operated from this address")
          }
        }
      }
    }

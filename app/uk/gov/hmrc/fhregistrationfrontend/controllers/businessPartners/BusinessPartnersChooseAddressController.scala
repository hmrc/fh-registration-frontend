...
import uk.gov.hmrc.fhregistrationfrontend.forms.businessPartners.BusinessPartnersChooseAddressForm.chooseAddressForm
...
class BusinessPartnersChooseAddressController @Inject()(
  ...
  def postAction(index: Int, mode: Mode): Call =
    routes.BusinessPartnersChooseAddressController.next(index, mode)
  ...
  def load(index: Int, mode: Mode): Action[AnyContent] = dataRequiredActionBusinessPartners(index, mode) {
    implicit request =>
      ...
      if (cachedAddressList.size <= 1)
        Redirect(routes.BusinessPartnersAddressController.load(index, mode))
      else {

...
import uk.gov.hmrc.fhregistrationfrontend.forms.businessPartners.BusinessPartnersUkAddressLookupForm.{businessPartnersUkAddressLookupForm, postcodeKey}
...
class BusinessPartnersCorporateBodyRegisteredAddressController @Inject()(
  ...
  val enterManualAddressUrl: String =
    routes.BusinessPartnersCorporateBodyEnterAddressController.load().url

  val backUrl: String = {
    if (hasVatNum) routes.BusinessPartnersPartnershipUtrController.load(1, NormalMode).url
